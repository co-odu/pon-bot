...
from database import (
    ...
    get_all_feedbacks, save_chat, get_all_chat_ids, remove_chat
)
...
broadcast_type_menu = ReplyKeyboardMarkup(
    keyboard=[
...
# ========== АВТО-СОХРАНЕНИЕ БЕСЕД (через my_chat_member) ==========

@dp.my_chat_member()
async def on_chat_member_update(update):
    """Сохраняет беседу, когда бота туда добавляют"""
    chat = update.chat
    if chat.type in ("group", "supergroup", "channel"):
        save_chat(chat.id, chat.title or "Без названия", chat.type)
        print(f"💾 Сохранена беседа: {chat.title} ({chat.id})")
...
async def handle_broadcast_button(message: Message):
    ...
    broadcast_state[message.from_user.id] = {"step": "choosing_type"}
    await message.answer(
        "📢 <b>Рассылка</b>\n\n"
        "Куда отправляем?",
        parse_mode=ParseMode.HTML,
...
@dp.message(lambda msg: msg.from_user.id in broadcast_state and broadcast_state[msg.from_user.id].get("step") == "choosing_type")
async def process_broadcast_type(message: Message):
    user_id = message.from_user.id
    text = message.text
    ...
    if text == "📨 Личные сообщения":
        broadcast_state[user_id] = {"step": "waiting_text", "target": "private"}
    elif text == "👥 Беседы":
        broadcast_state[user_id] = {"step": "waiting_text", "target": "chats"}
    else:
        await message.answer("❌ Выбери вариант из меню.", reply_markup=broadcast_type_menu)
        return
...
@dp.message(lambda msg: msg.from_user.id in broadcast_state and broadcast_state[msg.from_user.id].get("step") == "waiting_text")
async def process_broadcast(message: Message):
    ...
    if target == "private":
        all_targets = get_all_user_ids()
        target_name = "пользователям"
    else:
        all_targets = get_all_chat_ids()
        target_name = "беседам"
    
    sent = 0
    failed = 0
    
    await message.answer(f"📢 Начинаю рассылку {len(all_targets)} {target_name}...")
